...
from preliminary import get_sentence_embedding_bert
...
def compute_alignment_fusion_score(
    ...
    :param just_tokens_nf:证据句子未在embedding中找到的token
    :param idf_values:所有篇章中term的idf值
    :param query_matrix_bert:
    :param sentence_matrix_bert:
    :return:当前句子与问题的最终对齐分数
    """
    just_sent_matrix = just_matrix.transpose()
    match_score = np.matmul(ques_matrix, just_sent_matrix)
    ...
    score = math.log10(
        sum(final_just_score) * n_gram_max_mean_pooling(query_matrix_bert, sentence_matrix_bert)
    )
    return score


def n_gram_max_mean_pooling(query_matrix_bert, sentence_matrix_bert):
    score = []
    s_pq = np.dot(query_matrix_bert, sentence_matrix_bert.T)
    # 1-gram
    # score_1_gram = np.mean(np.max(s_pq, axis=1))
    # score.append(score_1_gram)

    # 2-gram,3-gram
    score_2_gram_list = []
    score_3_gram_list = []
    for i in range(s_pq.shape[0]):
        max_score_per_row_2_gram = 0
        max_score_per_row_3_gram = 0
        for j in range(s_pq.shape[1]):
            # 2-gram
            if j < s_pq.shape[1] - 1:
                if s_pq[i, j] + s_pq[i, j + 1] > max_score_per_row_2_gram:
                    max_score_per_row_2_gram = s_pq[i, j] + s_pq[i, j + 1]
            # 3-gram
            if j < s_pq.shape[1] - 2:
                if s_pq[i, j] + s_pq[i, j + 1] + s_pq[i, j + 2] > max_score_per_row_3_gram:
                    max_score_per_row_3_gram = s_pq[i, j] + s_pq[i, j + 1] + s_pq[i, j + 2]
        score_2_gram_list.append(max_score_per_row_2_gram / 2)
        score_3_gram_list.append(max_score_per_row_3_gram / 3)
    score_2_gram = np.mean(score_2_gram_list)
    score_3_gram = np.mean(score_3_gram_list)
    score.append(score_2_gram)
    score.append(score_3_gram)
...
def get_alignment_justification(
    ...
    for index_sentence, justification in enumerate(all_sentences):
        ...
        if just_tokens is not None:
            ...
            # 当前索引句子与问题的对齐分数
            index_score = compute_alignment_fusion_score(
                ques_matrix, ques_tokens_nf, ques_idf_mat, just_matrix, just_tokens_nf, idf_values,
                query_matrix_bert, sentences_matrix_bert[index_sentence]
            )
...
def one_iteration_block(final_indexes, first_iteration_index1, remaining_tokens1_3, ques_terms,
    ...
    if len(remaining_tokens1_3) <= 1:  # which can be considered as a very short query
...
def get_iterative_alignment_justifications(
        ...
        query_matrix_bert, sentences_matrix_bert, max_iteration=6, emb_size=300
):
    """
    ...
    for i in range(max_iteration):
        ...
        final_indexes, first_iteration_index1, remaining_tokens1 = one_iteration_block(
            ...
            all_sentences, embeddings_index, emb_size, idf_values, query_matrix_bert, sentences_matrix_bert)
...
def get_iterative_alignment_justifications_non_parametric_parallel_evidence(
        ques_terms, all_sentences, idf_values, embedding_index, query_matrix_bert, sentence_matrix_bert,
    ...
    for num_chain in range(parallel_evidence_num):
        ...
        # First iteration is here
        first_iteration_index1, remaining_tokens1, remaining_tokens2 = get_alignment_justification(
            ques_terms, all_sentences, embedding_index, emb_size, idf_values, query_matrix_bert, sentence_matrix_bert)
        ...
        for i in range(max_iteration):
            ...
            final_indexes, first_iteration_index1, remaining_tokens1 = one_iteration_block(
                ...
                embedding_index, emb_size, idf_values, query_matrix_bert, sentence_matrix_bert)
...
def dispose(context, question, all_sentences, all_sentences_matrix_bert, model, tokenizer,
            embeddings_index, emb_size, idf_values, bm25model, top_k=20):
    """
    按步实现无监督对齐迭代证据算法
    ...
    :param all_sentences:所有篇章中的句子（嵌套列表的形式）
    :param embeddings_index:嵌入表示
    :param all_sentences_matrix_bert:嵌入表示
    :param model:嵌入表示
    :param tokenizer:嵌入表示
    :param emb_size:嵌入表示维度大小，默认300维
    :param idf_values:
    ...
    query_matrix_bert = get_sentence_embedding_bert(model, tokenizer, question)
