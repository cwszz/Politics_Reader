...
from preliminary import read_txt
from preliminary import sentence_segmentation
from preliminary import get_idf
...
def get_alignment_justification(question_tokens, all_sentences, embeddings_index, emb_size, idf_values):
    """
    获得对齐的证据句子
    ...
    for index_article, justifications_per_article in enumerate(all_sentences):
        # 当前句子所在篇章内句子索引
        for index_sentence, justification in enumerate(justifications_per_article):
            sentences_list.append(justification)
            just_tokens = sentence_segmentation(justification, flag=1)
            if just_tokens is None:
                try:
                    all_sentences[index_article].remove(justification)
                except ValueError:
                    pass
                continue
    ...
    for i1 in second_iteration_index1:
        ...
        if i1 in final_indexes:
            ...
            if len(set(remaining_tokens1_3).intersection(set(
                    remaining_tokens2_all[index]))) == len(set(remaining_tokens1_3)):
                continue
                # return final_indexes, [], []
            # 否则添加当前句子为满足条件的当前迭代的最高分数句子
            final_indexes.append(i1)
            # 更新满足条件的当前迭代的最高分数句子对应的剩余未匹配项
            remaining_tokens1_4 = remaining_tokens2_all[index]
            break

    return final_indexes, second_iteration_index1, remaining_tokens1_4


def get_iterative_alignment_justifications(question_tokens, all_sentences, idf_values, embeddings_index,
...
def dispose(context, question, all_sentences, embeddings_index, emb_size):
    """
    按步实现无监督对齐迭代证据算法
    ...
    final_indexes = get_iterative_alignment_justifications(
        question_tokens, all_sentences, idf_values, embeddings_index, max_iteration=6, emb_size=emb_size)
    ...
    for final_index in final_indexes:
        index_article, index_sentence = get_index(final_index, all_sentences)
        # print(all_sentences[index_article][index_sentence])
        justifications.append(all_sentences[index_article][index_sentence])
...
if __name__ == '__main__':
    ...
    # dispose(read_json(), "壶兰计划是哪个市的？", sentences_partition(read_json()), embeddings, size)
    context = dispose(content, "壶兰计划是哪个市的？", sentences, embeddings, size)
    print(context)
